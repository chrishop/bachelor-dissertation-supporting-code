defmodule Mix.Tasks.MnistRun do
  ...
  def run(_) do
    ...
    IO.puts("epoch,train_accuracy,train_cost,validation_accuracy,validation_cost")
    Enum.each(
      1..epochs,
      fn ep_no ->
        epoch(ep_no,{train_inputs, train_labels, validation_inputs, validation_labels})
    end)
  ...
  defp epoch(ep_no, {train_inputs, train_labels, validation_inputs, validation_labels}) do
    {:ok, _new_network} = FeedForwardNetwork.train(train_inputs, train_labels, learning_rate: 0.005)

    train_input_sample = Enum.take(train_inputs, 1000)
    train_labels_sample = Enum.take(train_labels, 1000)
    {:ok, train_accuracy, train_cost} = FeedForwardNetwork.test(train_input_sample, train_labels_sample)
